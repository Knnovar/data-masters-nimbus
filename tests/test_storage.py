...
class TestParquet(unittest.TestCase):
    """Testes para escrita e promoção Parquet."""
    ...
    def test_write_parquet_creates_file(self):
        name = self.s.write_parquet("silver", "tb_test.csv", SAMPLE)
        self.assertEqual(name, "tb_test.parquet")
        self.assertTrue(self.s.exists("silver", "tb_test.parquet"))

    def test_write_parquet_correct_extension(self):
        name = self.s.write_parquet("silver", "tb_test.json", SAMPLE)
        self.assertEqual(name, "tb_test.parquet")

    def test_read_parquet_roundtrip(self):
        self.s.write_parquet("silver", "tb_test.csv", SAMPLE)
        loaded = self.s.read("silver", "tb_test.parquet")
        self.assertEqual(list(loaded.columns), list(SAMPLE.columns))
        self.assertEqual(len(loaded), len(SAMPLE))

    def test_parquet_preserves_schema(self):
        """Parquet preserva tipos — float nao vira string como no CSV."""
        self.s.write_parquet("silver", "tb_test.csv", SAMPLE)
        loaded = self.s.read("silver", "tb_test.parquet")
        # Coluna valor deve ser numerica (parquet nao converte para string)
        self.assertTrue(
            str(loaded["valor"].dtype).startswith("float") or
            str(loaded["valor"].dtype).startswith("int"),
            "Esperado tipo numerico, got: {}".format(loaded["valor"].dtype)
        )

    def test_promote_to_parquet_creates_parquet_in_silver(self):
        self.s.write("bronze", "tb_test.csv", SAMPLE)
        name = self.s.promote_to_parquet("tb_test.csv", "bronze", "silver")
        self.assertEqual(name, "tb_test.parquet")
        self.assertTrue(self.s.exists("silver", "tb_test.parquet"))

    def test_promote_to_parquet_archives_source(self):
        """Original vai para bronze/_archive/, nao e deletado."""
        self.s.write("bronze", "tb_test.csv", SAMPLE)
        self.s.promote_to_parquet("tb_test.csv", "bronze", "silver")
        # Arquivo nao existe mais no raiz do Bronze
        self.assertFalse(self.s.exists("bronze", "tb_test.csv"))
        # Mas existe no _archive/
        archive = self.tmp / "bronze" / "_archive" / "tb_test.csv"
        self.assertTrue(archive.exists(),
            "Original deveria estar arquivado em bronze/_archive/")

    def test_promote_to_parquet_data_integrity(self):
        self.s.write("bronze", "tb_test.csv", SAMPLE)
        name   = self.s.promote_to_parquet("tb_test.csv", "bronze", "silver")
    ...
    def test_parquet_smaller_than_csv_large_dataset(self):
        """Parquet com snappy deve ser menor que CSV em datasets reais (500+ linhas)."""
        import pandas as pd, numpy as np
        # Gera dataset com 500 linhas e repeticao de valores (favorece compressao)
        big_df = pd.DataFrame({
            "id"      : ["C{:04d}".format(i) for i in range(500)],
            "nome"    : ["Cliente {}".format(i % 50) for i in range(500)],
            "segmento": ["PRIME" if i % 3 == 0 else "VAREJO" for i in range(500)],
            "valor"   : [round(1000 + i * 10.5, 2) for i in range(500)],
        })
        self.s.write("bronze", "tb_big.csv", big_df)
        csv_size = (self.tmp / "bronze" / "tb_big.csv").stat().st_size
        self.s.write_parquet("silver", "tb_big.csv", big_df)
        pq_size = (self.tmp / "silver" / "tb_big.parquet").stat().st_size
        self.assertLess(
            pq_size, csv_size,
            "Parquet ({} bytes) deveria ser menor que CSV ({} bytes) em 500 linhas".format(
                pq_size, csv_size)
        )

    def test_list_includes_parquet(self):
        self.s.write_parquet("silver", "tb_test.csv", SAMPLE)
        files = self.s.list("silver")
        self.assertIn("tb_test.parquet", files)

    def test_promote_json_to_parquet(self):
        """promote_to_parquet funciona com qualquer formato de origem."""
        import json
        p = self.tmp / "bronze" / "tb_test.json"
        p.write_text(json.dumps({"data": SAMPLE.to_dict(orient="records")}))
        name = self.s.promote_to_parquet("tb_test.json", "bronze", "silver")
        self.assertEqual(name, "tb_test.parquet")
        self.assertTrue(self.s.exists("silver", "tb_test.parquet"))
