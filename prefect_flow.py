...
def task_profile(validated):
    ...
    try:
        ...
        parquet_filename = storage.promote_to_parquet(
            validated["filename"], "bronze", "silver"
        )
        _log("JOB-DM-003", "PROFILE/{}".format(table), "ENDED_OK",
             "rows={} ms={} promoted=bronze->silver parquet={}".format(
                 payload["rows"], payload["profiling_ms"], parquet_filename))
        return {**validated, "profiler_payload": payload, "silver_filename": parquet_filename}
    except Exception as e:
